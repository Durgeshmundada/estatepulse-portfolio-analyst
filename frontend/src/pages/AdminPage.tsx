...
export default function AdminPage() {
  ...
  return <main className="admin-shell">
    <header className="admin-header"><Link to="/chat" className="wordmark"><span className="brand-mark small">A</span> ASTRA <span className="admin-tag">BUSINESS</span></Link><div className="admin-auth"><input value={token} onChange={e => setToken(e.target.value)} type="password" placeholder="Admin token" /><button className="secondary" onClick={load}>Connect</button></div></header>
    <section className="admin-content">
      ...
      {detail && <section className="inspector"><header><button className="icon-button" onClick={() => { setDetail(null); setSelected(null) }}><ArrowLeft /></button><div><p className="eyebrow green">CONVERSATION INSPECTOR</p><h2>Messages & agent activity</h2></div></header><div className="inspector-grid"><div className="transcript">{detail.messages.map(m => <div className={`inspect-message ${m.role}`} key={m.id}><strong>{m.role === 'assistant' ? 'ASTRA' : 'User'}</strong><p>{m.text}</p>{m.cards.map((c, i) => <ResultCard key={i} card={c} readonly />)}</div>)}</div><div className="event-timeline"><h3><Activity size={17} /> Agent timeline</h3>{detail.events.map(e => <article key={e.id}><div className={`event-dot ${e.success ? '' : 'failed'}`} /><div><strong>{e.kind} · {e.name}</strong><span>{e.duration_ms} ms · {e.success ? 'Succeeded' : 'Fallback used'}</span><details><summary>Inspect payload</summary><pre>{JSON.stringify({ input: e.input, output: e.output, error: e.error }, null, 2)}</pre></details></div></article>)}</div></div></section>}
    </section>
  </main>
}
