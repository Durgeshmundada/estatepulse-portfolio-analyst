...
export default function ChatPage({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  return <main className="app-shell">
    ...
    <section className="chat-workspace">
      <header className="chat-header"><div className="header-left">{!sidebar && <button className="icon-button" onClick={() => setSidebar(true)}><Menu size={20} /></button>}<div><div className="agent-title"><span className="status-dot" /> ASTRA <span>Portfolio Analyst</span></div><small>Grounded in your portfolio data</small></div></div><div className="profile-menu-wrap"><button className="user-pill" onClick={() => setProfileMenu(open => !open)} aria-expanded={profileMenu} aria-haspopup="menu"><div className="user-initials">{user.name.split(' ').map(x => x[0]).join('')}</div><div><strong>{user.name}</strong><span>{user.city}</span></div><ChevronDown size={15} className={profileMenu ? 'rotated' : ''} /></button>{profileMenu && <div className="profile-menu" role="menu"><div><UserRoundCog size={16} /><span><strong>Demo portfolio</strong><small>{user.name} · {user.id}</small></span></div><button onClick={switchPortfolio} role="menuitem"><LogOut size={16} /> Switch portfolio</button></div>}</div></header>
